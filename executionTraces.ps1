	...
	do
		for mprob in ${mutationProbability[@]}
			do
				...
				# move to data files
				#mv "${path}/data/last release/executiontraces.txt" "${path}/data/last release/data/executiontraces-${iter}.txt";
...
Start-Process -FilePath 'java.exe' -NoNewWindow -Wait -ArgumentList @(
    '-jar .\CostBenefitOptimizer.jar'
    'block_file'
    'xprob'
    'mprob'
)

Move-Item .\Justatest.txt .\source\